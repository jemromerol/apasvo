...
from picking import stalta, ampa, takanami, findpeaks
...
class Check_sta_lta(unittest.TestCase):
	...
	def test_integer_instead_of_float_values_should_return_the_same(self):
		x = self.x.astype(int)
		et, cf = stalta.sta_lta(x, 50, sta_length=5, lta_length=100)
		fet, fcf = stalta.sta_lta(x.astype(np.float64), 50.0, sta_length=5.0, lta_length=100.0)
