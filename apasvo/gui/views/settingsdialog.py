...
class SettingsDialog(QtGui.QDialog):
    ...
    def loadSettings(self):
        ...
        key = int(settings.value('color_key', COLOR_KEYS.index(DEFAULT_COLOR_KEY)).toPyObject())
        self.colorKeyComboBox.setCurrentIndex(key)
        self._keyChanged(key)
        mColor = QtGui.QColor(str(settings.value(rc.method_stalta, DEFAULT_STALTA_COLOR).toPyObject()))
        self.colorMethodStaLtaButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.method_stalta_takanami, DEFAULT_STALTA_TAKANAMI_COLOR).toPyObject()))
        self.colorMethodStaLtaTakanamiButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.method_ampa, DEFAULT_AMPA_COLOR).toPyObject()))
        self.colorMethodAmpaButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.method_ampa_takanami, DEFAULT_AMPA_TAKANAMI_COLOR).toPyObject()))
        self.colorMethodAmpaTakanamiButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.method_takanami, DEFAULT_TAKANAMI_COLOR).toPyObject()))
        self.colorMethodTakanamiButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.method_other, DEFAULT_OTHER_COLOR).toPyObject()))
        self.colorMethodOtherButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.mode_manual, DEFAULT_MANUAL_COLOR).toPyObject()))
        self.colorModeManualButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.mode_automatic, DEFAULT_AUTOMATIC_COLOR).toPyObject()))
        self.colorModeAutomaticButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.status_preliminary, DEFAULT_PRELIMINARY_COLOR).toPyObject()))
        self.colorStatusPreliminaryButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.status_reviewed, DEFAULT_REVIEWED_COLOR).toPyObject()))
        self.colorStatusReviewedButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.status_confirmed, DEFAULT_CONFIRMED_COLOR).toPyObject()))
        self.colorStatusConfirmedButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.status_rejected, DEFAULT_REJECTED_COLOR).toPyObject()))
        self.colorStatusRejectedButton.setStyleSheet("background-color: %s" % mColor.name())
        mColor = QtGui.QColor(str(settings.value(rc.status_final, DEFAULT_FINAL_COLOR).toPyObject()))
        self.colorStatusFinalButton.setStyleSheet("background-color: %s" % mColor.name())
        settings.endGroup()

        settings.beginGroup("specgram_settings")
        windowlen = int(settings.value('window_len', SPECGRAM_WINDOW_LENGTHS[4]).toPyObject())
        ...
        self.noverlapSpinBox.setValue(int(settings.value('noverlap', windowlen / 2).toPyObject()))
        mwindow = str(settings.value('window', plotting.SPECGRAM_WINDOWS[2]).toPyObject())
