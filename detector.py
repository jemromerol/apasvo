...
def show(FILEIN, frequency=50.0, t_start=0.0, t_end=np.inf,
         show_cf=False, show_specgram=False, show_envelope=False,
         datatype='float64', byteorder='native', **kwargs):
    # Open file
    f = open(FILEIN, 'r') if utils.istextfile(FILEIN) else open(FILEIN, 'rb')
    # Load characteristic function
    cf = None
    if show_cf:
        print_msg("Loading Characteristic Function from %s... " % show_cf.name)
        fin_handler = utils.get_file_handler(show_cf, dtype=datatype, byteorder=byteorder)
        cf = fin_handler.read()
        print_msg("Done\n")
    # Load signal
    print_msg("Loading seismic signal from %s... " % f.name)
    record = picking.Record(f, frequency,
                            rfilename=f.name,
                            datatype=datatype)
    record.cf = cf
    print_msg("Done\n")
    # Plot
    print_msg("Showing signal...\n")
    fig = record.plot_signal(t_start=t_start, t_end=t_end,
                             show_cf=bool(show_cf),
                             show_events=False,
                             show_specgram=show_specgram,
                             show_envelope=show_envelope)
    pl.show(fig)
...
''' % (program_shortdesc, str(__date__))
    ...
    try:
        ...
        parser_generate.add_argument("FILEIN", nargs='*',
                                     action=GlobInputFilenames,
                                     help='''
        ...
        for each input file.
        ''')
        parser_generate.add_argument("--output-format",
                                     choices=["binary", "text"],
                                     default="binary",
                                     help='''
        Output file format. Default value is 'binary'.
        ''')
        parser_generate.add_argument("-n", "--n-events",
                                     type=positive_int,
                                     default=1,
                                     help='''
        Number of events generated, one file for each event.
        ''')
        parser_generate.add_argument("-f", "--frequency", type=positive_float,
                                     default=50.0,
                                     dest='fs',
                                     help="Signal frequency.")
        parser_generate.add_argument("-l", "--length",
                                     type=generated_sequence_length,
                                     default=600.0,
                                     help='''
        Length of the generated sequence. Default value is 600 seconds.
        ''')
        parser_generate.add_argument("-t", "--t-event",
                                     type=positive_float,
                                     required=True,
                                     help='''
        Point in time at which the event will be generated.
        ''')
        parser_generate.add_argument("-p", "--gen-event-power",
                                     type=float,
                                     default=5.0,
                                     help='''
        Power of the generated seismic event. Default value is 5 dB.
        ''')
        parser_generate.add_argument("--gen-noise-power",
                                     type=float,
                                     dest='P_noise_db',
                                     default=0.0,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-noise-coefficients",
                                     type=filein,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-low-period",
                                     type=positive_float,
                                     dest='low_period',
                                     default=50.0,
                                     help='''
        ...
        Default value is 50.
        ''')
        parser_generate.add_argument("--gen-high-period",
                                     type=positive_float,
                                     dest='high_period',
                                     default=10.0,
                                     help='''
        ...
        Default value is 10.
        ''')
        parser_generate.add_argument("--gen-bandwidth",
                                     type=positive_float,
                                     dest='bandwidth',
                                     default=4.0,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-overlap",
                                     type=positive_float,
                                     dest='overlap',
                                     default=1.0,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-f-low",
                                     type=positive_float,
                                     dest='f_low',
                                     default=2.0,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-f-high",
                                     type=positive_float,
                                     dest='f_high',
                                     default=18.0,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-low-amp",
                                     type=positive_float,
                                     dest='low_amp',
                                     default=0.2,
                                     help='''
        ...
        ''')
        parser_generate.add_argument("--gen-high-amp",
                                     type=positive_float,
                                     dest='high_amp',
                                     default=0.1,
                                     help='''
        ...
        # Create arguments for "show" command
        parser_show.add_argument("FILEIN", type=filein,
                                 help='''
        Binary or text file containing a seismic-like signal to be displayed.
        ''')
        parser_show.add_argument("--t-start", type=positive_float,
                                  default=argparse.SUPPRESS,
                                     help='''
        Starting point of the piece of displayed signal. If not specified then
        the signal will be displayed from the beginning.
        ''')
        parser_show.add_argument("--t-end", type=positive_float,
                                  default=argparse.SUPPRESS,
                                     help='''
        End point of the piece of displayed signal. If not specified then
        the signal will be displayed until the end.
        ''')
        parser_show.add_argument("-f", "--frequency", type=positive_float,
                                     default=50.0,
                                     help="Signal frequency.")
        parser_show.add_argument("--show-specgram", action='store_true',
                                     default=False, help='''
        Display spectrogram.
        ''')
        parser_show.add_argument("--show-envelope", action='store_true',
                                     default=False, help='''
        Display signal envelope.
        ''')
        parser_show.add_argument("--show-cf", type=argparse.FileType('r'),
                                     default=False, help='''
        File containing signal characteristic function to be displayed.
        ''')
